...
$(function llenarDatos() {
    $(document).on("change", "#selectActividades", function () {
        ...
        $.ajax({
            ...
            if (opcionSelect != 0) {
                $("#fecha").html(res.html.Fecha);
                $("#hora").html(res.html.Hora);
                $("#sede").html(res.html.Sede+"-"+res.Auditorio);
                $("#expositor").html(res.html.Nombre_Expositor);
                $("#descripcion").html(res.html.Detalle);
            }
        });
    });
});
