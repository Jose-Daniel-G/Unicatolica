...
if (!empty($_POST['funcion'])) {
    ...
    switch ($funcion) {
        ...
        case "cerrarSesion":
            session_unset();
            session_destroy();
            echo json_encode(['tipoRespuesta' => 'success']);
            break;

        default:
            echo json_encode(['tipoRespuesta' => 'error', 'mensaje' => 'Función no válida']);
            break;
    }
} else {
    echo json_encode(['tipoRespuesta' => 'error', 'mensaje' => 'Solicitud no válida']);
}
